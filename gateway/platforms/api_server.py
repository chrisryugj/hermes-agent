...
class APIServerAdapter(BasePlatformAdapter):
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        skip_context_files = _coerce_request_bool(body.get("skip_context_files"), default=False)
    ...
    def _get_codex_image_helpers(self):
        ...
        normalize_images = getattr(provider_module, "_normalize_input_images", None)
        if read_token is None or collect_b64 is None or normalize_images is None:
            return None, web.json_response(
                {"error": {"message": "openai-codex provider missing required helpers", "type": "server_error"}},
                status=503,
            )
        return (read_token, collect_b64, normalize_images), None
    ...
    async def _handle_image_generation(self, request: "web.Request") -> "web.Response":
        ...
        if err is not None:
            return err
        read_token, collect_b64, _normalize_images = helpers
    ...
    async def _handle_image_edit(self, request: "web.Request") -> "web.Response":
        ...
        if err is not None:
            return err
        read_token, collect_b64, normalize_images = helpers

        def _do_edit() -> str:
            token = read_token()
            if not token:
                raise RuntimeError("No Codex OAuth token available")
            # 새 plugin API는 정규화된 input_images 파트를 받는다. 업로드된 raw
            # base64를 data:image URL로 감싸 _normalize_input_images에 통과시키면
            # 실제 매직바이트에서 MIME을 재판별해 교정한다(잘못된 라벨 방지).
            input_images = normalize_images(f"data:image/png;base64,{image_b64}", None)
            b64 = collect_b64(
                token,
                prompt=prompt,
                size=mapped_size,
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Fork additions: image generation/edit + PDF render endpoints.
            ...
            # Dashboard plugin — fork addition.
            # Source of truth: <repo>/dashboard/ (git-tracked). Resolve relative
            # to this file so the fork's dashboard code is the ONLY copy consulted.
            # Override via HERMES_DASHBOARD_DIR env when needed.
            try:
                import sys
