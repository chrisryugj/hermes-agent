...
def _make_run_side_effect(
    ...
    # ``branch`` is upstream's name for the first parameter; accept both so
    # upstream-authored tests keep working against the fork's extended helper.
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_uses_main_tracking_remote_named_fork(
        ...
        assert any("fetch fork main" in cmd for cmd in commands)
        assert any("rev-list HEAD..fork/main --count" in cmd for cmd in commands)
        assert any("merge --ff-only fork/main" in cmd for cmd in commands)
        assert all("merge --ff-only origin/main" not in cmd for cmd in commands)
    ...
    def test_update_on_fork_checks_upstream_when_origin_up_to_date(
        ...
        # The fork detection goes through resolve_update_target(), which reads
        # the tracking remote's URL from git — steer it via remote_urls instead
        # of patching _get_origin_url (no longer consulted by the update path).
        mock_run.side_effect = _make_run_side_effect(
            branch="main",
            verify_ok=True,
            commit_count="0",
            remote_urls={"origin": "https://github.com/example/hermes-agent.git"},
        )
