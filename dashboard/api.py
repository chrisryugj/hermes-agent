...
def _require_auth(handler):
    """대시보드 관리 API는 게이트웨이와 같은 키로 인증한다.

    이 라우트들은 .env 쓰기·게이트웨이 재시작·cron 생성·세션 본문 열람이 가능해
    공개 터널 뒤에서 무인증으로 열려 있으면 안 된다.
    """
...
def register_dashboard_routes(app: "web.Application") -> None:
    ...
    get("/api/dashboard/fc-rag/queries", handle_fc_rag_queries)
    get("/api/dashboard/fc-rag/stats", handle_fc_rag_stats)
    get("/api/dashboard/fc-rag/trace/{trace_id}", handle_fc_rag_trace)
    # Chat menu
    get("/api/dashboard/chat/sessions", handle_chat_sessions)
    get("/api/dashboard/chat/sessions/{name}", handle_chat_session_detail)
    post("/api/dashboard/chat/send", handle_chat_send)
