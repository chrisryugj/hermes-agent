...
from hermes_cli.git_update import find_official_remote, resolve_update_target
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    # Fetch the official repo's main only. This sync compares the official main
    # against the fork's tracking target, so there's no reason to pull every
    # ref — a bare fetch drags in thousands of auto-generated branches.
    print()
    print(f"→ Fetching {official_remote}...")
    try:
        subprocess.run(
            git_cmd + ["fetch", official_remote, "main", "--quiet"],
            cwd=cwd,
            capture_output=True,
            check=True,
        )
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    branch = _m()._resolve_update_branch(args)
    update_target = resolve_update_target(git_cmd, _m().PROJECT_ROOT, branch=branch)
    ...
    try:
        ...
        # Scope the fetch to the branch we update against. A bare
        # `git fetch <remote>` pulls every ref, and this repo carries thousands
        # of auto-generated branches — an unscoped fetch can stall for minutes
        # on a non-single-branch checkout. Fetch only what we update against.
        print(f"→ Fetching updates from {update_target.remote}...")
        fetch_result = subprocess.run(
            git_cmd + ["fetch", update_target.remote, update_target.branch],
            cwd=_m().PROJECT_ROOT,
            capture_output=True,
            text=True, encoding="utf-8", errors="replace",
        )
        ...
        if current_branch != branch:
            ...
            if checkout_result.returncode != 0:
                ...
                # it up as a tracking branch of the resolved update target.
                # This is the common case when the requested branch exists
                # upstream but was never checked out locally.
                track_result = subprocess.run(
                    git_cmd + ["checkout", "-B", branch, update_target.ref],
                    cwd=_m().PROJECT_ROOT,
                    capture_output=True,
                    text=True, encoding="utf-8", errors="replace",
                )
                if track_result.returncode != 0:
                    ...
                    print(f"✗ Branch '{branch}' does not exist locally or on {update_target.remote}.")
        ...
        try:
            ...
            # `merge --ff-only <target.ref>` is byte-identical in effect to
            # `pull --ff-only <remote> <branch>` given the fresh tracking ref;
            # the divergence fallback below is unchanged.
            pull_result = subprocess.run(
                git_cmd + ["merge", "--ff-only", update_target.ref],
                cwd=_m().PROJECT_ROOT,
                capture_output=True,
                text=True, encoding="utf-8", errors="replace",
            )
