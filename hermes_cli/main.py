...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    target = resolve_update_target(git_cmd, PROJECT_ROOT, branch=branch)
    if target is None:
        print("✗ Could not determine which remote branch to compare against.")
        sys.exit(1)
    compare_branch = target.ref
    ...
    # so scope the fetch to the resolved target branch.
    print(f"→ Fetching from {target.remote}...")
    fetch_result = subprocess.run(
        git_cmd + ["fetch"] + depth_args + [target.remote, target.branch],
        cwd=PROJECT_ROOT,
        capture_output=True,
        text=True,
    )
