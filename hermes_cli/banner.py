...
def _check_via_local_git(repo_dir: Path) -> Optional[int]:
    ...
    # If the origin remote is the official repo over SSH, a local `git fetch`
    # can block on credential/host-key prompts. Probe the tip over the HTTPS
    # ls-remote path instead of fetching.
    ...
    # `rev-list --count HEAD..<target ref>` would report a huge bogus "behind"
    ...
    try:
        ...
        # An unscoped ``git fetch`` transfers every remote head (~1,400 on this
        # repo — measured 3.0 s vs 0.55 s scoped) and can burn the full 10 s
        # timeout on slow links. Modern git updates the remote-tracking ref on
        # a scoped fetch, so the ``HEAD..<target.ref>`` count below is
        # unaffected; the shallow path compares against FETCH_HEAD, which a
        # scoped fetch also updates.
        fetch_args = ["git", "fetch", target.remote, target.branch]
        if is_shallow:
            fetch_args += ["--depth", "1"]
    ...
    if is_shallow:
        ...
        target_rev = (
            ...
            or _git_stdout(["rev-parse", target.ref], cwd=repo_dir)
        )
...
def check_for_updates() -> Optional[int]:
    ...
    # Read cache — invalidate if the embedded rev, the local repo target, OR
    # the installed version changed since the last check.
    now = time.time()
    try:
