...
def _check_via_local_git(repo_dir: Path) -> Optional[int]:
    ...
    try:
        fetch_args = ["git", "fetch", target.remote]
        if is_shallow:
            fetch_args += ["--depth", "1"]
...
def check_for_updates() -> Optional[int]:
    ...
    # Read cache — invalidate if the embedded rev, the local repo target, OR the
    # installed version changed since the last check. The version guard matters
    ...
    # None); without it the stale "behind" count would survive the upgrade for
    # up to 6h. See #34491.
    now = time.time()
    try:
